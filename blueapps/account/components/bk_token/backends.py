...
class TokenBackend(ModelBackend):
    def authenticate(self, request=None, bk_token=None):
        ...
        try:
            ...
            # ITSM-增加Profile
            user.set_property(key='profile',
                              value=json.dumps(user_info.get('profile', {})))

            # ITSM-避免一个微信账号重复绑定到多个蓝鲸账号，后面的绑定会自动解绑前面的绑定
            wx_userid = user_info.get('wx_userid', '')
            if wx_userid:
                # 从其他用户身上解绑
                UserProperty.objects.filter(
                    key='wx_userid', value=wx_userid
                ).exclude(user=user).update(value='')

            user.save()
        except IntegrityError:
            logger.exception(traceback.format_exc())
            logger.exception(
                "get_or_create UserModel fail or update_or_create UserProperty"
            )
            return None
        except Exception:
            logger.exception(traceback.format_exc())
    ...
    @staticmethod
    def get_bk_user_profile(bk_token, user):
        """获取用户资料：组织架构信息
        """

        basic_profile = {
            'message': 'get_user_profile api call failed, return default profile',
            'name': user['chname'],
            'phone': user['phone'],
            'email': user['email'],
            'departments': []
        ...
        # TODO-特殊逻辑
        def convert_for_xietong(raw_data):
            """身份数据转换：协同办公版本"""
            return {
                ...
                'departments': [
                    ...
                    for unit in raw_data.get('profile', {}).get('units', [])]
            }

        api_params = {'bk_token': bk_token}

        try:
            response = send(ConfFixture.USER_PROFILE_URL, 'GET', api_params,
                            verify=False)
        except Exception:
            return basic_profile

        if response and response.get('result', False):
            return convert_for_xietong(response.get('data'))
    ...
    def get_user_info(bk_token):
        ...
        if response.get('result') is True:
            ...
            # 更新用户资料信息
            user_info['profile'] = TokenBackend.get_bk_user_profile(bk_token, user_info)

            return True, user_info
        else:
            error_msg = response.get('message', '')
            error_data = response.get('data', '')
            logger.error("Failed to Get User Info: error=%(err)s, ret=%(ret)s"
                         % {
                             'err': error_msg,
                             'ret': error_data,
                         })
    ...
    def verify_bk_token(bk_token):
        ...
        if response.get('result'):
            ...
            error_msg = response.get('message', '')
            error_data = response.get('data', '')
            logger.error("Fail to verify bk_token, error={}, ret={}".format(
